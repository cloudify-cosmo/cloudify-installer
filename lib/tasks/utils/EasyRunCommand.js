...
function printOutput( level ){
    var totalResp = [];
    var resp = "";
    return function(buffer){
        resp += buffer.toString();
        var args = resp.split('\n');
        totalResp = totalResp.concat(args);
        _.each(_.compact(_.initial(args) ), function (arg) {
            _logger[level](arg.trim());
        });
        resp = _.last(args);
    };
}
...
function run_cmd( opts, callback ) {
    ...
    try {
        ...
        child.stdout.on('data', printOutput('info'));

        child.stderr.on('data', printOutput('error'));

        //child.stderr.on('data', function (buffer) { resp += buffer.toString(); });
        child.stdout.on('end', function () {
            callback(null, ''); // totalResp.join('\n'));
        });
    } catch (e) {
        callback(e);
    }
